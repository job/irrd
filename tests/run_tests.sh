...
function tests_with_hiding {
    (   echo "admin_citesting"
    sleep 1
    echo "config"
    sleep 1
    echo "access-list 99 deny all"
    sleep 1
    echo "irr_database sampledb cryptpw-access 99"
    sleep 1
    echo "exit"; sleep 1
    echo "exit"; sleep 1 ) | telnet localhost 5673 || echo
    tests_round_1
}
...
function rebootirrd {
    ...
    (   echo "admin_citesting"
        sleep 2
        echo "reboot"
        sleep 1 ) | telnet localhost 5673 || echo
    sleep 5
}
